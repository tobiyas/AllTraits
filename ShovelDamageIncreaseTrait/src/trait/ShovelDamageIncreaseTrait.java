...
public class ShovelDamageIncreaseTrait extends AbstractPassiveTrait{
	...
	private boolean checkItemIsSpade(ItemStack stack){
		Material item = stack.getType();
		if(item == Material.WOOD_SPADE)
			return true;
		
		if(item == Material.STONE_SPADE)
			return true;
		
		if(item == Material.GOLD_SPADE)
			return true;
		
		if(item == Material.IRON_SPADE)
			return true;
	...
	public boolean isBetterThan(Trait trait) {
		if(!(trait instanceof ShovelDamageIncreaseTrait)) return false;
		ShovelDamageIncreaseTrait otherTrait = (ShovelDamageIncreaseTrait) trait;
