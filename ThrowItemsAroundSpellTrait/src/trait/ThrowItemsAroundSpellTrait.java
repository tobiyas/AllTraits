...
public class ThrowItemsAroundSpellTrait extends AbstractMagicSpellTrait implements Listener {
	...
	@TraitConfigurationNeeded( fields = {
			...
			@TraitConfigurationField(fieldName = "split", classToExpect = Double.class, optional = true),
	...
	public void setConfiguration(TraitConfiguration configMap) throws TraitConfigurationFailedException {
		...
		if(configMap.containsKey("split")){
			split = configMap.getAsDouble("split");
		}
	...
	protected Vector randomize(Vector vec){
		double off = vec.length() / split;
		
		double xOffset = (rand.nextDouble() * off) - (off / 2);
		double yOffset = (rand.nextDouble() * off) - (off / 2);
		double zOffset = (rand.nextDouble() * off) - (off / 2);
