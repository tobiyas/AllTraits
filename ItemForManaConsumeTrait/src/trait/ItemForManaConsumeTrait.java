...
public class ItemForManaConsumeTrait extends AbstractMagicSpellTrait {
	...
	protected void magicSpellTriggered(Player player, TraitResults result) {		
		if(plugin.getPlayerManager().getSpellManagerOfPlayer(player.getName()).getManaManager().isManaFull()){
		...
		plugin.getPlayerManager().getSpellManagerOfPlayer(player.getName()).getManaManager().fillMana(newValue);
