...
public class MagicAreaHealTrait extends AbstractActivateAETrait {
	...
	protected ParticleContainer particles = new ParticleContainer(de.tobiyas.racesandclasses.vollotile.ParticleEffects.HEART, 1, 0);
	
	protected ParticleContainer particlesFromSelf = null;
	
	
	public MagicAreaHealTrait() {
	}
	...
	protected boolean triggerOnEntity(RaCPlayer player, Entity otherEntity) {
		...
		if(modHeal <= 0 || event.isCancelled()) return false;
		
		CompatibilityModifier.LivingEntity.safeHealEntity((LivingEntity) otherEntity, modHeal);
		if(particles != null) Vollotile.get().sendOwnParticleEffectToAll(particles, otherEntity.getLocation());
